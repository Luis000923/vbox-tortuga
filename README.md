...
```
 ...
 3) Revertir cambios / Reactivar Hyper-V
 Q) Salir
```
...
- **Opción 1**: si detecta BitLocker activo, primero te muestra/guarda la clave de
  ...
  Guard/Hyper-V y ajusta el arranque (`bcdedit hypervisorlaunchtype off`). Antes de
  empezar, avisa en pantalla si detecta que VT-x/AMD-V está apagado en la BIOS/UEFI.
...
- **Opción 3**: revierte la opción 1 — reactiva VBS/HVCI/Credential Guard, las
  características de Hyper-V y el hipervisor en el arranque (`bcdedit
  hypervisorlaunchtype auto`). Útil si alternas VirtualBox con Docker Desktop,
  WSL2 o Windows Sandbox.

En ambos casos (1 y 2) se te pide **consentimiento explícito** antes de enviar
cualquier dato. Puedes negarte y el script funciona igual, solo que sin
reportar nada — y sin ese "S" explícito, el script sigue funcionando pero
**no manda nada a la API** (es normal, no un error).
...
| Docker / WSL2 / Sandbox dejaron de funcionar | Usa la **opción 3** del menú (revierte Hyper-V/VBS) |

Si nada de esto lo resuelve, usa la **opción 2** del menú para reportarlo.

## Windows Defender marca el `irm | iex` como sospechoso

Es un falso positivo común: cualquier comando que descarga y ejecuta un script
en una sola línea (`irm ... | iex`) dispara detección heurística de SmartScreen
/ Defender, aunque el contenido sea legítimo. Si ves un aviso:

- **SmartScreen ("Windows protegió su PC")**: clic en **Más información** →
  **Ejecutar de todas formas**.
- **Directiva de ejecución de PowerShell**: el script ya usa
  `-ExecutionPolicy Bypass` al autoelevarse, pero si la Opción B (archivo local)
  te bloquea con `... no se puede cargar porque la ejecución de scripts está
  deshabilitada en este sistema`, ejecuta en su lugar:
  ```powershell
  powershell -ExecutionPolicy Bypass -File .\Quitar-Tortuga-VBox.ps1
  ```
- Si tu antivirus corporativo bloquea la descarga por completo, usa la
  **Opción B** (descarga el `.ps1` del repositorio) en vez del one-liner.
